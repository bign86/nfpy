...
__version__ = '0.1'
_TITLE_ = "<<< Re-download single asset script >>>"
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if not downloads:
            print(f'{Ut.Col.WARNING.value}Supplied ticker does not exist! Please give a valid one.{Ut.Col.ENDC.value}')
        else:
            break
    ...
    for i in idx:
        dwn = downloads[i]
        dwnf.run_download(do_save=True, override_date=True,
                          provider=dwn.provider, page=dwn.page, ticker=ticker,
