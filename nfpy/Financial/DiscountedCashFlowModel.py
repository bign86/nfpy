...
from nfpy.Financial.FundamentalsFactory import FundamentalsFactory
...
class DCFResult(AttributizedDict):
...
class DiscountedCashFlowModel(BaseFundamentalModel):
    ...
    def _calc_tax_rate(self, array: np.array):
        ...
        tax_rate = np.maximum(np.minimum(tax_rate, self._MAX_TAX_R), self._MIN_TAX_R)
        array[7, :y] = tax_rate
    ...
    def _calculate(self):
        ...
        array[10, :y] = np.array([compound(self._idx.expct_return(
            start=pd.Timestamp(dt.year, 1, 1), end=dt),
            BDAYS_IN_1Y) for dt in index[:y]])
