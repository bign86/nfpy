...
class BaseFundamentalResult(AttributizedDict):
...
class BaseFundamentalModel(metaclass=ABCMeta):
    ...
    def __init__(self, company: str, date: Union[str, pd.Timestamp] = None,
        ...
        if not isinstance(self._cmp, Company):
            raise AssetTypeError('Uid {} is not a Company'.format(company))
        ...
        self._start = self._t0 - pd.Timedelta(days=DAYS_IN_1Y * self._ph)
