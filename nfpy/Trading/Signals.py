...
def sma(v: pd.Series, w: int = 21) -> pd.Series:
    ...
    m = round(.8 * w)
    ret = v.rolling(w, min_periods=m).mean()
...
def csma(v: pd.Series, w: int = 21) -> pd.Series:
    ...
    m = round(.8 * w)
    ret = v.expanding(w, min_periods=m).mean()
...
def ewma(v: pd.Series, w: int = 21, span: int = None) -> pd.Series:
    ...
    m = round(.8 * w)
    ret = v.ewm(span=span, min_periods=m, ignore_na=True).mean()
...
def macd(v: pd.Series, w_fast: int = 12, w_slow: int = 26, w_macd: int = 9) -> tuple:
