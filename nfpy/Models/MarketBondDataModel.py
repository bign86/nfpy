...
class MarketBondDataModel(MarketAssetsDataBaseModel):
    ...
    def _calc_bond_math(self):
        ...
        prices[9] += p_vola_6m
        prices[10] -= p_vola_6m
        arr_ytm, _ = Math.calc_ytm(
            dates, asset.inception_date.asm8,
        ...
        arr_fv, _ = Math.calc_fv(
            t0.asm8, asset.inception_date.asm8,
        ...
        self._res_update(
            yields=ytm,
            yields_array=arr_ytm[:, :7],
            ytm_bars=arr_ytm[:, 7:],
            last_ytm=last_ytm,
            fair_values=pd.DataFrame(arr_cf, index=rates, columns=cols),
            convexity=asset.convexity(t0),
            duration=asset.duration(t0)
        )
